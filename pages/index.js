...
import Banner from "../public/images/dashboard.jpg";
...
const Home = ({
  ...
  const handlePosts = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/blog/fetch-all-post`
      );
      console.log("Data", data.data);
      if (data.success) {
        setPostsContent(data.data);
      }
    } catch (error) {
  ...
  console.log("featurePost", postsContent);
  ...
  return (
    <Base>
      {/* Banner */}
      <section className="section banner relative m-4 pb-0">
        {/* <ImageFallback
                      ...
                      <div className="md:col-6">
                        {postsContent ? (
                          postsContent.map((post, index) => (
                            <div className="mb-4 md:col-3" key={index}>
                              <BlogCard post={post} />
                            </div>
                          ))
                        ) : (
